...
function saveGameState() {
  try {
    ...
    localStorage.setItem(key, JSON.stringify({
      wordle: wordleState,
      trivia: triviaState,
      cw: { userGrid: cwState.userGrid, checked: cwState.checked }
    }));
  } catch(e) {}
}
...
function loadGameState() {
  try {
    ...
    if (s.wordle) { wordleState = Object.assign({ guesses: [], current: "", over: false, won: false }, s.wordle); wordleState.current = ""; }
    if (s.trivia) triviaState = Object.assign({ answered: false, selected: null }, s.trivia);
    if (s.cw) { cwState.userGrid = s.cw.userGrid || {}; cwState.checked = s.cw.checked || {}; }
  } catch(e) {}
}
...
function updateCrosswordCell(cellIndex, value) {
  ...
  if (value !== "" && value !== "#" && !/[A-Z]/.test(ch)) return;
  g[cellIndex] = ch || "#";
  draft.games.crossword.grid = g.join("");
  // Update cell appearance in place without full re-render
  var cell = document.querySelector(".cw-editor-cell:nth-child(" + (cellIndex+1) + ")");
  if (cell) {
    var isBlack = (ch === "#" || !ch);
    cell.className = "cw-editor-cell" + (isBlack ? " cw-black" : "");
    var inp = cell.querySelector("input");
    if (inp) inp.style.color = isBlack ? "#aaa" : "#111";
  }
  // Re-render just the clue inputs after a short delay (debounced)
  clearTimeout(window._cwRenderTimer);
  window._cwRenderTimer = setTimeout(function() { render(); }, 800);
}
